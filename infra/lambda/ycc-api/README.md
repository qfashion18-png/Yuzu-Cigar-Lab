...
- `POST /support/email-send` currently remains guarded unless `FEATURE_SES=ready`; AWS SES production access was final-denied under case `177809591700724`, so keep it non-ready until outbound mail is migrated to an approved sender.
- `POST /newsletter/subscribe` upserts a public newsletter subscriber, monthly membership interest, preferred tier, selected cigar brands, matched promoted cigars, source, and audit row. Subscriber follow-up email remains guarded until outbound mail is migrated to an approved sender; replies should go to `SUPPORT_EMAIL_INBOUND_RECIPIENT`.
...
- Member welcome email is intentionally sent after membership activation, not after Cognito account confirmation. Friends & Family Box Pass claims send the branded benefits/tips welcome after the entitlement is granted; Stripe `customer.subscription.created` webhooks send it after an active subscription is stored. The response includes `memberWelcomeEmail.status` as `sent`, `pending_ses`, `failed`, or `skipped`.
- Do not create per-customer SES recipient identities from signup, post-confirmation, membership activation, or newsletter flows. In SES sandbox, recipient identity verification sends an AWS-branded verification message to the customer and is not a Yuzu onboarding experience. It is only a temporary operator/test exception for known inboxes. Production customer mail must use the branded Cognito verification template for account confirmation and a replacement approved transactional sender for support, newsletter, and member welcome email.
...
`POST /newsletter/subscribe` sends subscriber-facing follow-up only when outbound mail is explicitly enabled; because AWS SES production access was final-denied under case `177809591700724`, the signup still succeeds and reports the follow-up as pending until a replacement sender is live. The selected-cigar promotion email includes text and branded HTML bodies so clients can render product images, public costs, member costs, and shop links while still preserving a plain-text fallback. Member welcome email uses the same guarded outbound path, so live activation events report `pending_ses` until outbound transactional mail is migrated to an approved sender. Per-customer SES recipient verification must not be used as an onboarding step because it sends AWS-branded verification email and does not solve production delivery. `POST /support/contact` is public but only sends to the configured YCC support recipient (`SUPPORT_CONTACT_EMAIL_TO`, falling back to `SUPPORT_EMAIL_FROM`) when outbound mail is ready and sets Reply-To to the visitor. `POST /support/email-send` is restricted to Cognito `admin` and `concierge_operator` groups. SES receipt events are also accepted directly from the active receipt rule set for inbound processing; the handler reads raw email from `SUPPORT_EMAIL_RAW_BUCKET` and `SUPPORT_EMAIL_RAW_PREFIX`, routes every inbound email through `YCCSupportAgent`, then persists the inbound support case, received email, and an operator-review outbound draft.

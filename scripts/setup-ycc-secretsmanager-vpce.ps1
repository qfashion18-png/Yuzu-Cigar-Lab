...
$endpointGroupName = "ycc-secretsmanager-vpce"
$endpointServiceName = "com.amazonaws.$Region.secretsmanager"
...
if ($null -eq $endpointSg) {
  $createdGroup = Invoke-AwsJson @(
    "ec2", "create-security-group",
    "--group-name", $endpointGroupName,
    "--description", "YCC Secrets Manager VPC endpoint access from Lambda",
    "--vpc-id", $vpcId
  )
...
$ingressPath = Join-Path $env:TEMP "ycc-secretsmanager-vpce-ingress.json"
$egressPath = Join-Path $env:TEMP "ycc-secretsmanager-vpce-egress.json"
...
[
  {
    ...
    "UserIdGroupPairs": [
      {
        "GroupId": "$lambdaSgId",
        "Description": "Allow Lambda to call Secrets Manager endpoint"
      }
    ]
  }
]
    ...
    "UserIdGroupPairs": [
      {
        ...
        "Description": "Allow Lambda to reach Secrets Manager VPC endpoint"
      }
    ]
  }
]
